...
public interface ProductDao extends JpaRepository<Product, Integer> {
	...
	@Query(value="SELECT * FROM Products WHERE Status = 1 AND Id In (SELECT Product_Id From Product_Cate WHERE Cate_Id = ?) order by ManuDay DESC", nativeQuery = true)
	Page<Product> fillShopDESC(int number1 , Pageable pageable);
	
	@Query(value="SELECT * FROM Products WHERE Status = 1 AND Id In (SELECT Product_Id From Product_Cate WHERE Cate_Id = ?) order by Price DESC", nativeQuery = true)
	Page<Product> fillPriceHigh(int number1 , Pageable pageable);
	
	@Query(value="SELECT * FROM Products WHERE Status = 1 AND Id In (SELECT Product_Id From Product_Cate WHERE Cate_Id = ?) order by Price ASC", nativeQuery = true)
	Page<Product> fillPriceLow(int number1 , Pageable pageable);
